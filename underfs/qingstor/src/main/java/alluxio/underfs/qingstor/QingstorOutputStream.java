...
/**
 * A stream for writing a file into Qingstor. The data will be persisted to a temporary directory on
 * the local disk and copied as a complete file when the {@link #close()} method is called.
 */
public class QingstorOutputStream extends OutputStream {
  private static final Logger LOG = LoggerFactory.getLogger(QingstorOutputStream.class);

  /** The Bucket of Qingstor for object operations. */
  ...
  /**
   * Creates a name instance of {@link QingstorOutputStream}.
   *
   * @param key the key of the file
   * @param bucket
   * @throws IOException if an I/O error occurs
   */
  public QingstorOutputStream(Bucket bucket, String key) throws IOException {
    Preconditions.checkArgument(key != null && !key.isEmpty(),
        "Qingstor path must not be null or empty.");
    Preconditions.checkArgument(bucket != null, "Qingstor bucket must not be null.");
   ...
   * uploaded to Qingstor. Once the file is uploaded, the temporary file is deleted.
   *
   * @throws IOException if an I/O error occurs
   */
  @Override
  public void close() throws IOException {
    ...
    try {
      ...
      input.setBodyInputFile(mFile);
      input.setContentLength(mFile.length());
      if (mHash != null) {
        byte[] hashBytes = mHash.digest();
        input.setContentMD5(new String(Base64.encodeBase64(hashBytes)));
      }
      mBucket.putObject(mKey, input);
      if (!mFile.delete()) {
